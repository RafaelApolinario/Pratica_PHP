...
<?php 
require_once "conexao.php"; //arquivo que contem conexão como banco de dados
$conexao = novaConexao(); // dados para fazer conexão como banco de dados $servidor,$usuario,$senha,$banco

if($_GET['codigo']) { 
//$_GET['codigo'] = exemplo de como vai estar a URL http://localhost/exercicio.php?dir=bancoDeDados&file=alterar&codigo=1
...
if(count($_POST) > 0) {
    ...
    if (!count($erros)){ // se nao tiver nenhum erro 
        
        $sql = "UPDATE cadastro 
        SET nome=?, nascimento=?, email=?, site=?, filhos=?, salario=? 
        WHERE id = ?" ; // prepara a query

        $stmt = $conexao->prepare($sql); // prepare=Prepara uma instrução para execução e retorna um objeto de instrução
        ...
        $stmt->bind_param("ssssidi", ...$params);//bind_param — Passa variáveis para um preparado comando como parâmetros
...
<?php foreach($erros as $erro): ?>
    ...
    <!-- </div> -->
<?php endforeach ?>
<!-- http://localhost/exercicio.php?dir=bancoDeDados&file=alterar&codigo=1 -->
